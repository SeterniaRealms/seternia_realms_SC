...
use anchor_spl::token::{MintTo ,mint_to ,Burn ,burn,Transfer,transfer};
...
pub fn mint(
    ...
    let race: Box<Account<'_, Race>> = accts.race.clone();
    ...
    let amount: u64 = race.value.clone();
    let name: String = race.name.clone();
    let seeds: &[&[&[u8]]] = &[&[
        TRESURE_SEED,
        binding.as_ref()
    ]];
    if race.class != class.key() {
        return Err(CustomError::IncorrectDefinition.into());
    }
    if race.limit <= race.current {
        return Err(CustomError::OrderFilled.into());
    }
    if level > 0{
    ...
    if is_nft == true{
        ...
        burn(burn_ctx, 1)?;
    }else if accts.rune_accont.amount >= amount && amount > 0 {
        if accts.admin_ata_destination.owner != authority.admin{
            return Err(CustomError::IncorrectAssociated.into());
        }
        if accts.rune_accont.mint != accts.race.token{
            return Err(CustomError::IncorrectDefinition.into());
        }
    ...
    let _ = create_metadata_accounts_v3(
        createmeta,
        DataV2{
            name: format!("{name} {title} Level: {level} #{id}"),
...
pub struct MintNFT<'info> {
    ...
    #[account(mut)]
    pub admin_ata_destination: Account<'info, TokenAccount>,
    ...
    /// CHECK:
    pub rune_mint: UncheckedAccount<'info>,
