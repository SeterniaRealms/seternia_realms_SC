...
pub fn create_race(ctx: Context<CreateRace>,id:u64,limit:u64,name:String,token:Pubkey,value:u64) -> Result<()> {
    ...
    let accts: &mut CreateRace<'_> = ctx.accounts;
    let authority: &mut Box<Account<'_, Treasure>> = &mut accts.treasure;
    ...
    accts.race.class = accts.class.key();
    accts.race.limit = limit;
    accts.race.token = token;
    accts.race.value = value;
    accts.race.name = name;
    Ok(())
}
pub fn create_trait(ctx: Context<CreateTrait>,id:u64,level:u64,coins:u64,uri:String) -> Result<()> {
    ...
    accts.traits.race = accts.race.key();
...
pub struct CreateTrait<'info> {
    ...
    #[account(
        ...
        seeds = [race.key().as_ref(),id.to_le_bytes().as_ref()], 
        bump,
    )]
