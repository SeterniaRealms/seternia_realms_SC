...
import { MintData, Treasure } from "./require"
...
describe("seternia", () => {
  ...
  it("should mint and active NFT successfully!", async () => {
    ...
    let cump_limit = ComputeBudgetProgram.setComputeUnitLimit({ units: 2800_000 });
    ...
    if(await con.getAccountInfo(AdminTokenAccount)==null){
      const ToAdminToken = createAssociatedTokenAccountInstruction(
        wallet.publicKey,
        AdminTokenAccount,
        treasury_data.admin,
        TOKEN_OUT,
      ...
      transaction = transaction.add(ToAdminToken)
